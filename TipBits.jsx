...
function LightningRain({ active, onDone }) {
  ...
  useEffect(() => {
    ...
    const bolts = Array.from({ length: 32 }, (_, i) => ({
      ...
      color: Math.random() > 0.4 ? "#F7931A" : "#fb923c",
      delay: i * 35,
    }));
    ...
    const animate = (ts) => {
      ...
      bolts.forEach((b) => {
        ...
        if (b.y < H + b.size) {
          allDone = false;
          drawBolt(ctx, b.x, b.y, b.size, b.opacity * fade * fadeOut, b.rotation, b.color);
        }
      });
  ...
  return (
    <canvas ref={canvasRef} style={{
      ...
      pointerEvents: "none", zIndex: 10, borderRadius: 16,
    }} />
  );
}
...
export default function TipBits() {
  ...
  const generateInvoice = async () => {
    ...
    if (!address || !address.includes("@")) {
      setError("No Lightning address configured. Set VITE_LIGHTNING_ADDRESS in your environment.");
      return;
    }
    ...
    try {
      ...
      if (!paramsRes.ok) throw new Error("Could not reach Lightning address. Check it is correct.");
      ...
      if (memo.trim()) {
        callbackUrl.searchParams.set("comment", memo.trim().slice(0, 144));
      }
    ...
    <div style={{
      ...
      <div className={`tj-wrap ${mounted ? "in" : ""}`} style={{ width:"100%", maxWidth:430, position:"relative", zIndex:1 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
          ...
          <button className="nav-link" onClick={() => window.dispatchEvent(new CustomEvent('tipbits-nav', { detail: 'how' }))}>
            How it works
          </button>
        </div>
        ...
        <div style={{ background:"white", border:"1.5px solid #e5e7eb", borderRadius:16, padding:"26px 24px", position:"relative", overflow:"hidden", boxShadow:"0 4px 32px rgba(0,0,0,.06),0 1px 4px rgba(0,0,0,.04)" }}>
          ...
          {step === "choose" && (<>
            ...
            <div style={{ position:"relative", marginBottom:18 }}>
              {currency !== "SATS" && (
                <span style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", color:"#9ca3af", fontSize:14, pointerEvents:"none" }}>
                  {cur.symbol}
                </span>
              )}
            ...
            <textarea className="tj-input" placeholder="Leave a note..."
              ...
              maxLength={144}
              rows={2} style={{ resize:"none", marginBottom:20, lineHeight:1.6 }} />

            <div className="conversion-box">
              <div>
                <div style={{ fontSize:11, color:"#9ca3af", marginBottom:3, textTransform:"uppercase", letterSpacing:"0.06em" }}>You're sending</div>
                {currency !== "SATS" && (
                  <div style={{ fontSize:12, color:"#6b7280" }}>{cur.symbol}{(parseFloat(inputVal)||0).toFixed(2)} {currency}</div>
                )}
                {currency === "SATS" && fiatEquiv(satsAmount) && (
                  <div style={{ fontSize:12, color:"#6b7280" }}>{fiatEquiv(satsAmount)}</div>
                )}
              </div>
            ...
            <button className="btn-primary" onClick={generateInvoice}
              disabled={loading || !satsAmount || satsAmount < 1}>
              {loading
                ? <><span className="spin">⚡</span> Generating invoice...</>
          ...
          {step === "invoice" && (<>
            <div style={{ textAlign:"center" }} className="success-ring">
              ...
              <div style={{ display:"flex", alignItems:"baseline", justifyContent:"center", gap:6, marginBottom:4 }}>
                <span style={{ fontSize:30, fontWeight:700, color:"#F7931A", fontFamily:"'IBM Plex Mono',monospace" }}>
                  {satsAmount.toLocaleString()}
                </span>
                <span style={{ fontSize:13, color:"#9ca3af" }}>sats</span>
              </div>
              {currency !== "SATS" && (
                <div style={{ fontSize:13, color:"#6b7280", marginBottom:3 }}>{cur.symbol}{parseFloat(inputVal).toFixed(2)} {currency}</div>
              )}
              {currency === "SATS" && fiatEquiv(satsAmount) && (
                <div style={{ fontSize:13, color:"#6b7280", marginBottom:3 }}>{fiatEquiv(satsAmount)}</div>
              )}
            ...
            <div style={{ display:"flex", gap:8, marginTop:12 }}>
              ...
              <button className={`btn-copy ${copied ? "done" : ""}`} onClick={copyInvoice}>
